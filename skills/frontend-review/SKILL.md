...
**排除**：`*-cases.html` 等设计探索文件不纳入审查范围（cases 是 ui-cases 生成的选型文件，不是正式实现）。

## Step 2: Gather Context

1. 读取 `.impeccable.md` — 项目设计上下文（品牌、调性、设计原则）
2. 读取 `design-spec.md` — 冻结的设计规范（如果有）。Reviewer 会将实现与 spec 对比，偏离冻结规范视为 Important issue。
3. 读取变更文件的完整上下文（不只是 diff）
4. 读取 `frontend-design/SKILL.md` — 设计原则和 DON'T list（用于 reviewer 判定 AI slop 和评分校准）
